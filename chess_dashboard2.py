...
# --- Plot 8: Top Openings by Game Length ---
opening_turns = chess.groupby("opening_shortname")["turns"].mean().sort_values(ascending=False).reset_index()
opening_turns.columns = ["opening_shortname", "avg_turns"]
length = opening_turns.head(20)

fig_line = px.line(length, x="opening_shortname", y="avg_turns",
                   title="Top Openings Across Game Length Categories",
                   labels={"opening_shortname": "Opening", "avg_turns": "Average Turns"},
                   markers=True,
                   color_discrete_sequence=["#FDB813"])
fig_line.update_layout(template="plotly_dark", xaxis_tickangle=-45)
st.subheader("Top Openings with Longest Average Game Length")
st.plotly_chart(fig_line, use_container_width=True)

# --- Plot 9: Top Openings by Average Moves ---
opening_moves = chess.groupby("opening_shortname")[["opening_moves"]].agg("mean").reset_index()
opening_moves.columns = ["opening_shortname", "avg_moves"]
top_moves = opening_moves.sort_values(by="avg_moves", ascending=False).head(10)

fig_moves = px.bar(top_moves, x="opening_shortname", y="avg_moves",
                   title="Longest Openings by Average Number of Moves",
                   labels={"opening_shortname": "Opening", "avg_moves": "Average Moves"},
                   color="avg_moves", color_continuous_scale="RdPu")
fig_moves.update_layout(template="plotly_dark", xaxis_tickangle=-45)
st.subheader("Top Openings by Average Number of Moves")
st.plotly_chart(fig_moves, use_container_width=True)

# --- Plot 10: Top Players by Win Rate ---
